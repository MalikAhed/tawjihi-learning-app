...
- Use `assets/icons/brain-svgrepo-com.svg` to identify dry facts that students must memorize, paired with the Arabic label `يجب حفظه`. Reserve this brain illustration for memorization content.
- Use the first Access summary (`src/ui/lesson/access-summary.js` and its `.access-*` rules in `src/styles/lesson/published.css`) as the approved lesson visual reference: crisp, simple SVG illustrations; pale thematic cards with soft 2px borders, modest corners and no bottom popout shadow; accent-matched bullet dots; compact but readable Arabic spacing. Highlight exact recall terms sparingly. Adapt each lesson's composition, diagrams and palette to its content rather than copying this layout. Main questions should test priority facts; retain supplementary questions and stable IDs for future side quests.
