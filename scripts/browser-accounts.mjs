...
export async function verifyAccountJourney({ appUrl, navigate, evaluate, waitFor, assert, cdp, sessionId, captureScreenshot, delay, completeRoadmap }) {
  ...
  await waitFor("Boolean(document.querySelector('[data-onboarding-step=\"5\"]:not([hidden])'))", "the required phone question");
  assert(await evaluate(`(() => {
    const input=document.querySelector('input[name=phone]');
    const picker=document.querySelector('select[name=phonePrefix]');
    const inputBox=input.getBoundingClientRect();
    const pickerBox=picker.getBoundingClientRect();
    return input.required && !document.querySelector('[data-skip-field=phone]') && picker.options.length === 3 && pickerBox.right < inputBox.left;
  })()`), "phone must be required and its three-option prefix picker must sit left of the number field");
  await cdp.send("Emulation.setDeviceMetricsOverride", { width:390, height:568, deviceScaleFactor:1, mobile:true }, sessionId);
  assert(await evaluate("document.documentElement.scrollWidth <= innerWidth && document.documentElement.scrollHeight <= innerHeight && document.querySelector('select[name=phonePrefix]').getBoundingClientRect().left >= 0"), "the complete phone step must remain contained without scrolling on mobile");
  await captureScreenshot("account-phone-required-mobile.png");
  await cdp.send("Emulation.setDeviceMetricsOverride", { width:1440, height:900, deviceScaleFactor:1, mobile:false }, sessionId);
  await evaluate("document.querySelector('select[name=phonePrefix]').value='+972'; document.querySelector('input[name=phone]').value='598932239'; document.querySelector('input[name=phone]').focus()");
  ...
  await waitFor("getComputedStyle(document.querySelector('.subject-card[data-subject=ict]')).transform === 'matrix(1, 0, 0, 1, 0, 2)'", "the shared slight lesson-style press on hover");
