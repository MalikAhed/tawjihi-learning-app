...
const PUBLIC_EXTENSIONS = new Set([".css", ".js", ".mjs", ".json", ".png", ".jpg", ".jpeg", ".svg", ".webp", ".ttf", ".woff", ".woff2"]);
...
export function resolvePublicFilePath(root, urlPath) {
  ...
  const normalizedPath = path.posix.normalize(`/${decoded.replace(/^\/+/, "")}`);
  const isApplicationFile = normalizedPath.startsWith("/assets/")
    || (normalizedPath.startsWith("/src/") && !normalizedPath.startsWith("/src/server/"));
  const isPublic = normalizedPath === "/" || normalizedPath === "/index.html"
    || PUBLIC_DEPENDENCIES.has(normalizedPath)
    || (isApplicationFile && PUBLIC_EXTENSIONS.has(path.posix.extname(normalizedPath).toLowerCase()));
  if (!isPublic) return null;
  const relative = normalizedPath === "/" ? "index.html" : normalizedPath.replace(/^\/+/, "");
  const resolved = path.resolve(root, relative);
  return resolved === root || resolved.startsWith(`${root}${path.sep}`) ? resolved : null;
}

export async function createStaticFileHandler({ root, htmlInjection = "" } = {}) {
  ...
  return async function handleStaticFile(request, response) {
    ...
    try {
      ...
      if (extension === ".html") {
        ...
        response.end(request.method === "HEAD" ? undefined : html.replace("</body>", `${htmlInjection}</body>`));
        return;
      }
