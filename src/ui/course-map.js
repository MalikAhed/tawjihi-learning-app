...
function subjectCardMarkup(subject, view) {
  ...
  return `<button class="subject-card${locked ? " subject-card--locked" : ""}" type="button" data-subject="${escapeHtml(subject.id)}" data-status="${view.state}" aria-label="${escapeHtml(view.label)}"${locked ? " data-expanded-component" : ""}${disabledMarkup}><span class="subject-card__hero"><span class="subject-card__title"><h2>${escapeHtml(subject.name)}</h2></span></span><span class="subject-card__details"><span class="subject-card__progress-track" data-subject-progress data-label-covered="${isProgressLabelCovered(view.progress, 100)}"${locked ? " hidden" : ""}><span aria-hidden="true"></span><bdi class="subject-card__progress-count ui-number" data-subject-count>${view.completedLessons} / ${view.totalLessons}</bdi></span></span>${locked ? LOCK_ICON : ""}</button>`;
}
