...
import { getLessonUiCopy, lessonChoiceMarker, localizedDefault, resolveLessonContent } from "./lesson-ui-copy.js";
...
function renderMascotHeader(config, className, locale, copy) {
  ...
  return `<header class="lesson-question-header ${className}-head"><div class="lesson-question-heading"><h1 id="ui-lab-content-title">${escapeHtml(config.title)}</h1><p>${escapeHtml(config.prompt)}</p></div><div class="lesson-question-mascot"><div class="lesson-question-bubble">${escapeHtml(mascot)}</div><img src="assets/mascot/rocky-standing-still-reduced.svg" alt="${escapeHtml(copy.friendlyMascot)}" /></div></header>`;
}
...
function footerOptions(type, config, locale) {
  const copy = getLessonUiCopy(locale);
  if (type === "mcq") return { feedback:"", feedbackAttributes:{ "data-ui-lab-feedback":true, "aria-live":"polite" }, primaryLabel:copy.checkAnswer, primaryAttributes:{ "data-ui-lab-check":true, disabled:true }, primaryLabelAttributes:{ "data-ui-lab-check-label":true } };
