...
function lessonGroup(unit, lesson, { getPartProgress, getAccess, currentPart }) {
  ...
  return `<section class="roadmap-lesson-group"${lesson.optional ? " data-expanded-component" : ""}><header class="roadmap-lesson-divider"><div class="roadmap-lesson-heading"><h3>${escapeHtml(lesson.label)}${lesson.recommended ? '<span class="roadmap-recommended">موصى به</span>' : ""}</h3></div></header><ol class="roadmap-parts" aria-label="أجزاء ${escapeHtml(lesson.label)}">${stops}</ol></section>`;
}
